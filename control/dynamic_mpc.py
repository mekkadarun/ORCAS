...
class DynamicMPC:
    ...
    def optimize_trajectory(self, current_state, goal, obstacles):
        """Convex formulation with linearized dynamic constraints"""
        ...
        # Linearized obstacle constraints (DCP-compliant)
        for t in range(1, self.horizon + 1):
            for obs in obstacles:
                if isinstance(obs, DynamicObstacle):
                    pred_positions = obs.get_predicted_positions(self.dt, self.horizon)
                    for pred_pos in pred_positions:
                        # Current relative position vector
                        rel_pos = current_state[:2] - pred_pos
                        ...
                        constraints += [
                            dir_vec @ (pos[t] - pred_pos) >= min_dist
                        ]
